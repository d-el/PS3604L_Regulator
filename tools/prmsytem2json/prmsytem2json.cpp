...
int main(int argc, char *argv[]) {
    ...
    while(!buffer.empty()) {
        ...
        quote_opened = delimiter == '\'' && !quote_opened;
        //std::cout << "    " << string_parsing::pad_right('[' + token + ']', 16)
        //    << "   " << delimiter << '\n';
	...
	if(mode == "type"){
		...
		for(auto group = parameterstree.begin(); group != parameterstree.end(); group++){
			...
			parameters_t &parameters = group->parameters;
			for(const auto& param : parameters){
				out << "\tN" << param.name << " = " << n++ << "u,\n";
			}
		}
		out << "\n\tendOfNumberPrm = " << n++ << "u\n";
		out << "} parametresNum_type;\n\n";
		...
		for(auto group = parameterstree.begin(); group != parameterstree.end(); group++){
			...
			parameters_t &parameters = group->parameters;
			for(const auto& param : parameters){
				out << "\ttype" << param.type << " " << param.name << ";\n";
			}
		}
		out << "} prmData_type;\n\n";
	}
	...
	if(mode == "object"){
		...
		for(auto group = parameterstree.begin(); group != parameterstree.end(); ){
			...
			for(const auto& param : parameters){
				out << "\t{\n";
				out << "\t\t.prm = (prmval_type*)&prmData." << param.name << ",\n";
				out << "\t\t.def.t_" << param.type << "= " << param.def << " * power" << param.pow << ",\n";
				out << "\t\t.min.t_" << param.type << "= " << param.min << " * power" << param.pow << ",\n";
				out << "\t\t.max.t_" << param.type << "= " << param.max << " * power" << param.pow << ",\n";
				out << "\t\t.label = \"" << param.name << "\",\n";
				out << "\t\t.units = " << param.unit << ",\n";
				out << "\t\t.pCallback = " << param.callback << ",\n";
				out << "\t\t.addr = " << param.addr << ",\n";
				out << "\t\t.parameter = " << param.param << ",\n";
				out << "\t\t.type = " << param.type << ",\n";
				out << "\t\t.chmod = " << param.chmod << ",\n";
				out << "\t\t.power = " << param.pow << ",\n";
				out << "\t\t.save = " << param.savetype << ",\n";
				out << "\t},\n";
			}
			++group;
		}
