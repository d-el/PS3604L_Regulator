...
static unsigned int compute_response_length_from_request(modbus_t *ctx, uint8_t *req)
{
    ...
    case MODBUS_FC_READ_FILE_RECORD:
    	length = 1 + 1 + 1 + 1 + 2 * (req[offset + 7] << 8 | req[offset + 8]);
    	break;
    case MODBUS_FC_WRITE_FILE_RECORD:
		length = 1 + 1 + 1 + 2 + 2 + 2 + 2 * (req[offset + 7] << 8 | req[offset + 8]);
		break;
...
/* Read File Record */
int modbus_read_file_record(modbus_t *ctx,
                            int file_number, int record_number,
                            int record_len, uint16_t *dest)

{
    ...
    req_length = 0;
    req[req_length++] = ctx->slave;
    req[req_length++] = MODBUS_FC_READ_FILE_RECORD;
    req[req_length++] = 0x07; // Byte Count
	req[req_length++] = 0x06;  // Reference Type
	req[req_length++] = file_number >> 8;
	req[req_length++] = file_number & 0x00FF;
	req[req_length++] = record_number >> 8;
	req[req_length++] = record_number & 0x00FF;
	req[req_length++] = record_len >> 8;
	req[req_length++] = record_len & 0x00FF;
...

{
    ...
    req_length = 0;
    req[req_length++] = ctx->slave;
    req[req_length++] = MODBUS_FC_WRITE_FILE_RECORD;
    req[req_length++] = 0x07 + record_len * 2; // Request data length
	req[req_length++] = 0x06;  // Reference Type
	req[req_length++] = file_number >> 8;
	req[req_length++] = file_number & 0x00FF;
	req[req_length++] = record_number >> 8;
