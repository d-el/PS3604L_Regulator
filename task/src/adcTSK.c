...
void adcTSK(void *pPrm){
    ...
    while(1){
        ...
        while(adcreg < adcregEnd){
            ...
            */
            filter->recursiveFilterOut = iq_filtr(filter->recursiveFilterCumul, val, filter->recursiveK);

            /**************************************
            * Заполнение массивов фильтра скользящего среднего
            */
            filter->MA_filterMas[filter->MA_filterIndex++] = filter->recursiveFilterOut;
            ...
            */
            *filtered = movingAverageFilter(&filter->MA_filterMas[0], filter->MA_filter_WITH);
            filter++;
            filtered++;
            adcreg++;
        }
		...
		*/
        if(a->adcIna226 <= s->pIEx[1].adc){
        	a->currentIna226 = s32iq_Fy_x1x2y1y2x(s->pIEx[0].adc, s->pIEx[1].adc, s->pIEx[0].qi, s->pIEx[1].qi, a->adcIna226);
        }else{
        	a->currentIna226 = s32iq_Fy_x1x2y1y2x(s->pIEx[1].adc, s->pIEx[2].adc, s->pIEx[1].qi, s->pIEx[2].qi, a->adcIna226);
        }
        if(a->currentIna226 < 0) a->currentIna226 = 0;
        ...
        */
        if(a->filtered[CH_UADC] <= s->pU[1].adc){
        	...
        	a->voltage = s32iq_Fy_x1x2y1y2x(s->pU[1].adc, s->pU[2].adc, s->pU[1].qu, s->pU[2].qu, a->filtered[CH_UADC]);
        }else{
        	a->voltage = s32iq_Fy_x1x2y1y2x(s->pU[2].adc, s->pU[3].adc, s->pU[2].qu, s->pU[3].qu, a->filtered[CH_UADC]);
        }
        ...
        */
        if(a->filtered[CH_IADC] <= s->pU[1].adc){
        	a->currentInt = s32iq_Fy_x1x2y1y2x(s->pI[0].adc, s->pI[1].adc, s->pI[0].qi, s->pI[1].qi, a->filtered[CH_IADC]);
        }else if(a->filtered[CH_IADC] <= s->pU[2].adc){
        	a->currentInt = s32iq_Fy_x1x2y1y2x(s->pI[1].adc, s->pI[2].adc, s->pI[1].qi, s->pI[2].qi, a->filtered[CH_IADC]);
        }else{
        	a->currentInt = s32iq_Fy_x1x2y1y2x(s->pI[2].adc, s->pI[3].adc, s->pI[2].qi, s->pI[3].qi, a->filtered[CH_IADC]);
        }
        if(a->currentInt < 0)  a->currentInt = 0;
        ...
        */
        a->radPower  = _IQ14mpy(_IQtoIQ14(a->udc - a->voltage), _IQtoIQ14(a->current));
...
static inline void adcTaskStctInit(void){
    for(uint8_t ch = 0; ch < ADC_NUM_CH; ch++){
        adcTaskStct.adcFilt[ch].recursiveFilterCumul = adcTaskStct.adcFilt[ch].adcDefVal << adcTaskStct.adcFilt[ch].recursiveK;
