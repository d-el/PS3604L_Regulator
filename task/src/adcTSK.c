...
adcTaskStct_type adcTaskStct = {
	{
		{ .adcDefVal = 0, .oversampling = 1, .recursiveK = 1, .MA_filter_WITH = 16, },	//U_MEAS
		{ .adcDefVal = 0, .oversampling = 1, .recursiveK = 1, .MA_filter_WITH = 16, },  //I_MEAS
		{ .adcDefVal = 0, .oversampling = 1, .recursiveK = 1, .MA_filter_WITH = 16, },  //UDC_MEAS
	},
...
void adcTSK(void *pPrm){
	...
	while(1){
		...
		while(adcreg < adcregEnd){
			 ...
			 */
			filter->MA_filterMas[filter->MA_filterIndex++] =
					filter->recursiveFilterOut;
			 ...
			 */
			*filtered = movingAverageFilter(&filter->MA_filterMas[0],
					filter->MA_filter_WITH);
			filter++;
			filtered++;
			adcreg++;
		}
		 ...
		 */
		a->udc = a->filtered[CH_UINADC]
				* _IQ((AdcVref * (UDC_Rh + UDC_Rl)) / (65536 * UDC_Rl));
		...
		if(rg.tf.state.bit.switchIsON != 0){
			a->outPower = _IQ14mpy(_IQtoIQ14(a->voltage),
					_IQtoIQ14(a->current));
		}else{
			a->outPower = 0;
		}
